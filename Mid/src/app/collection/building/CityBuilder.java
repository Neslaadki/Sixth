...
public class CityBuilder implements ICityBuilder<City>, MessageErrors {
    ...
    public CityBuilder() throws IOException {
            this.input = new ConsoleReader();

        this.governorBuilder = new GovernorBuilder();
        this.cityCoordinateBuilder = new CityCoordinateBuilder();
    }
    ...
    @Override
    public void setArea() {
        System.out.println(System.lineSeparator() + "Введите площадь города, целочисленное значение " + "(не превосходит " + Long.MAX_VALUE + ")" + System.lineSeparator());
        input.getDefaultConsoleSymbol();
        String line = input.getScanner().nextLine().trim();
        try {
            if (Long.parseLong(line) > 0 && Long.parseLong(line) < Long.MAX_VALUE) {
    ...
    public void setMetersAboveSeaLevel() {
        System.out.println(System.lineSeparator() + "Введите значение высоты над уровнем моря, целочисленное значение " + " (не превосходит " + Long.MAX_VALUE + ")");
        input.getDefaultConsoleSymbol();
        String line = input.getScanner().nextLine().trim();
        try {
            if (Long.parseLong(line) > 0 && Long.parseLong(line) < Long.MAX_VALUE) {
    ...
    public void setClimate() {
        ...
        try {
            if (Integer.parseInt(line.trim()) == 0) {
                ...
                System.out.println("Тип уровня жизни в городе успешно выбран: " + climate.getRussianName());
            }
        } catch (Exception e) {
            System.out.println(messageOfInputFormatError);
            setClimate();
    ...
    public void setGovernment() {
        ...
        try {
            if (Integer.parseInt(line.trim()) == 0) {
                ...
                System.out.println("Тип уровня жизни в городе успешно выбран: " + government.getRussianName());
            }
        } catch (Exception e) {
            System.out.println(messageOfInputFormatError);
            setGovernment();
    ...
    public void setStandartOfLiving() {
        ...
        try {
            ...
            System.out.println("Тип уровня жизни в городе успешно выбран: " + standardOfLiving.getRussianName());
        } catch (Exception e) {
            System.out.println(messageOfInputFormatError);
            setStandartOfLiving();
        }
